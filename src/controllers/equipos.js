...
function index(req, res) {
  // with placeholder
  connection.query(` select e.*,j.nombre as nombre_jugadores,e2.nombre as nombre_entrendores , e3.nombre as nombre_estadios,p.nombre as nombre_paises ,c.nombre as nombre_ciudades from equipos e
...
function update(req, res) {
 ...
 const id_ciudades=req.body.id_ciudades;
  ...
  connection.query(
    `update equipos SET nombre=?,fecha_fundo=?,id_jugadores=?,id_entrenadores=?,descripcion=?,simbolo=?,indumentaria_uniforme=?,presidente=?,apodos=?,id_estadios=?,id_paises=?,id_ciudades where id=?;`,
    [nombre,fecha_fundo,id_jugadores,id_entrenadores,descripcion,simbolo,indumentaria_uniforme,presidente,apodos,id_estadios,id_paises,id_ciudades,id],
